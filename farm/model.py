    ...
    def call(self, X, if_sparse_max=False, valid_lens=None, **kwargs):
        ...
        if valid_lens == None:
            if if_sparse_max == True:
                # shape_X = tf.shape(X)  # [B, Q, K] 或 [N, K]
                # X2d = tf.reshape(X, (-1, shape_X[-1]))  # -> [B*Q, K]
                # Y2d = tfa.activations.sparsemax(X2d)  # sparsemax 需要 2D
                # return tf.reshape(Y2d, shape_X)  # 还原形状
...
class masked_softmax_random(tf.keras.layers.Layer):
    ...
    def call(self, X, **kwargs):
        ...
        random_indices = tf.random.uniform(shape=[shape_X[1] * self.top_k], minval=0, maxval=self.maxval,
...
class dotproductattention(tf.keras.layers.Layer):  # @save
    ...
    def call(self, queries, keys, values, relative_encoding_lookup=None, edge_type_embedding=None,
             if_select_feature=None, **kwargs):
        d = queries.shape[-1]
        ...
        if relative_encoding_lookup == None:
            scores = tf.matmul(queries, keys, transpose_b=True) / tf.math.sqrt(tf.cast(shape[-1], dtype=tf.float32))
            ...
            scores_ = tf.matmul(queries, keys, transpose_b=True) * 0.6
            # print("scores_ shape")
            # print(scores_.shape)
            ...
            scores = scores / tf.math.sqrt(tf.cast(shape[-1], dtype=tf.float32))
            # print(scores.shape)
...
class encoder_block(tf.keras.layers.Layer):
    ...
    def call(self, X, if_sparse_max=False, enc_valid_lens=None, relative_pos_enc=None, edge_type_enc=None, **kwargs):
        ...
        score, value, query, keys = self.dotproductattention(X, X, X, relative_encoding_lookup=relative_pos_enc,
                                                             edge_type_embedding=edge_type_enc)
...
class decoder_cross_block(tf.keras.layers.Layer):
    ...
    def __init__(self, num_hiddens, if_select_feature_=None, if_self_att_=False):
        ...
        self.dotproductattention_deco_cross = dotproductattention(num_hiddens, if_select_feature=if_select_feature_,
                                                                  if_self_att=if_self_att_)
        self.att_embedding = attention_embedding(num_hiddens)
        self.r_connection = residual_connection()

    def call(self, Y, X, if_sparse_max=False, enc_valid_lens=None, if_select_feature_=None, **kwargs):
        if if_select_feature_ == None:
            score_deco_cross, value_deco_cross, query_deco_cross, key_deco_cross = self.dotproductattention_deco_cross(
                Y, X, X, if_select_feature=if_select_feature_)
        else:
            score_deco_cross, query_deco_cross, key_deco_cross = self.dotproductattention_deco_cross(Y, X, X,
                                                                                                     if_select_feature=if_select_feature_)
...
class feature_selection_cross_block(tf.keras.layers.Layer):
    """
    Define global feature selection cross attention block
    """
    ...
    def call(self, Y, X, gene_embedding, mutation_embedding, if_sparse_max=False, enc_valid_lens=None,
             if_select_feature_=None, **kwargs):
        if if_select_feature_ == None:
            score_deco_cross, value_deco_cross, query_deco_cross, key_deco_cross = self.dotproductattention_deco_cross(
                Y, X, X,
                gene_embedding=gene_embedding, mutation_embedding=mutation_embedding,
                if_select_feature=if_select_feature_)
        else:
            score_deco_cross, query_deco_cross, key_deco_cross = self.dotproductattention_deco_cross(Y, X, X,
        ...
        att_embedding_deco_cross_gene = tf.math.add(key_deco_cross, gene_embedding)

        att_embedding_deco_cross_gene = tf.math.add(att_embedding_deco_cross_gene, mutation_embedding)

        return query_deco_cross, att_score_deco_cross, att_embedding_deco_cross_gene
...
class drug_transformer_():
    ...
    def __init__(self, gene_embeddings):  # , relative_pos_enc_lookup=None):
        ...
        self.dense_b_name = tf.keras.layers.Dense(60, kernel_initializer=initializers.RandomNormal(seed=42),
                                             activation='relu',
                                             kernel_regularizer=regularizers.L2(1e-4),
                                             bias_initializer=initializers.Zeros(), name="dense_b_name")
        ...
        self.dense_5 = tf.keras.layers.Dense(1, activation='sigmoid',
        ...
        self.pos_encoding_gene = positionalencoding(30, 1962)
    ...
    def temp_model(self):
        ...
        gene_embedding = tf.broadcast_to(gene_embedding,
                                         [shape_input[0], gene_embedding.shape[1], gene_embedding.shape[-1]])
    ...
    def model_construction_midi(self, if_mutation=None):
        ...
        """
        X_input = Input((85, 8))
        MIC_input = Input((11)) # 11 -> 9
        Y_input = Input((1962, 2))
        gene_mutation_input = Input((1962, 2))
        rel_position_embedding = Input((85, 85, 60))
        edge_type_embedding = Input((85, 85, 5))
        ...
        # processing the one-hot of bacteria name
        b_name_input = tf.expand_dims(MIC_input, axis=1)
        b_name_shape = tf.shape(b_name_input)
        b_name_input = tf.broadcast_to(b_name_input,
                                       [b_name_shape[0], 1962, b_name_shape[-1]])

        b_name_input = tf.math.l2_normalize(self.dense_b_name(b_name_input), axis=-1)
        ...
        # add the bacteria name embedding
        Y = tf.math.add(Y, b_name_input)
        """
        ...
        X_global, att_score_global1, Y_value = self.decoder_global_1(X_global, Y,
                                                                     if_sparse_max=False)  # , if_select_feature_=None)
        X_global, att_score_global2, Y_key = self.decoder_global_2(X_global, Y_value, if_sparse_max=False,
                                                                   if_select_feature_=True)
        # X_global, att_score_global1, Y_value = self.decoder_global_1(
        #     X_global, Y, if_sparse_max=True, enc_valid_lens=enc_valid_lens_)
        #
        # X_global, att_score_global2, Y_key = self.decoder_global_2(
        #     X_global, Y_value, if_sparse_max=True, enc_valid_lens=enc_valid_lens_, if_select_feature_=True)
        ...
        self.model = Model(inputs=(
            X_input, Y_input, enc_valid_lens_, MIC_input, rel_position_embedding, edge_type_embedding,
            gene_mutation_input),
            outputs=Y)
        # self.model.compile(loss= "mean_squared_error" , optimizer="adam", metrics=["mean_squared_error"])
...
def return_drug_gene(CCLE_name_test, drug_name_list_test, gene_expression_test, drug_one_hot_encoding_test,
                     drug_smile_length_test, ic50_list_test, drug_smile_list_test, drug_name):
    lung_index = []
    ...
    return np.array(drug_one_hot_encoding_test_lung), np.array(gene_expression_test_lung), \
        np.array(
            drug_smile_length_test_lung), drug_name_lung, CCLE_name_test_lung, ic50_list_test_lung, drug_smile_list_test_lung
...
def return_gene_smile_target(model, drug_one_hot, gene_expression, drug_length, batch_size=30):
    ...
    for i in range(fraction_):
        feature_select_score1 = feature_select_score_model1.predict((drug_one_hot[i * batch_size:(i + 1) * batch_size],
                                                                     gene_expression[
                                                                     i * batch_size:(i + 1) * batch_size], np.array(
            drug_length[i * batch_size:(i + 1) * batch_size])))[1]
...
def return_gene_drug_target_train(model, gene_names, drug_lung, gene_lung, drug_lung_length, CCLE_name_lung,
                                  drug_name_lung, drug_smile_lung, ic50_lung, top_gene=5370, batch_size=10):
    """
    ...
    for i in range(faction_):
        feature_select_score1 = feature_select_score_model1.predict((drug_lung[i * batch_size:(i + 1) * batch_size],
                                                                     gene_lung[i * batch_size:(i + 1) * batch_size],
                                                                     np.array(drug_lung_length[
        ...
        feature_select_score = tf.concat([feature_select_score1[1], feature_select_score2[1], feature_select_score3[1]],
                                         axis=1)
